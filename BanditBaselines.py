    ...
    def chooseArmToPlay(self):
        ...
        for arm in self.nbArms:
            ...
            if self.nbDraws[arm] > 0:
                reward_contribution = self.cumRewards[arm] / self.nbDraws[arm]
            else:
                reward_contribution = self.cumRewards[arm] # safety
            
            ucb_contribution = np.power(self.alpha * np.log(self.t)/self.nbDraws[arm], 0.5) 
            ucb = reward_contribution + ucb_contribution
            if ucb > max_ucb:
                max_ucb = ucb
                A_t = arm
        return A_t
